...
public class Main {
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> new LoginWindow().setVisible(true));  // Start the login window
    }
}
...
class MainMenu extends JFrame {
    ...
    class AddCompanyFrame extends JFrame {
        public AddCompanyFrame() {
            ...
            addButton.addActionListener(e -> addCompany(nameField, locationField));
        }

        private void addCompany(JTextField nameField, JTextField locationField) {
            Connection connection = DBConnection.getConnection();
            if (connection == null) {
                JOptionPane.showMessageDialog(this, "Failed to connect to the database.");
                return;
            ...
            try {
                String insertQuery = "INSERT INTO company (name, location) VALUES (?, ?)";
                PreparedStatement preparedStatement = connection.prepareStatement(insertQuery);
    ...
    class AddUserFrame extends JFrame {
        ...
        private void addUser(JTextField usernameField, JTextField roleField) {
            ...
            try {
                String insertQuery = "INSERT INTO users (username, role) VALUES (?, ?)";
                PreparedStatement preparedStatement = connection.prepareStatement(insertQuery);
    ...
    class ViewAdvanceSalaryFrame extends JFrame {
        ...
        private void displayAdvanceSalary(int employeeId, JTextArea advanceSalaryArea) {
            ...
            try {
                ...
                String selectQuery = "SELECT employee_id, advance_amount, date_issued, status " +
                        "FROM advance_salary " +
                        "WHERE employee_id = ?";
                PreparedStatement preparedStatement = connection.prepareStatement(selectQuery);
                preparedStatement.setInt(1, employeeId);
